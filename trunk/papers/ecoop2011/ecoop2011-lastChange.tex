...
We introduce a new practical functionality in debuggers called \textit{lastChange}, 
which automatically locates the last point that a variable or an object property 
has been changed and provides related information such as the call stack and program state. Starting from a 
...
re-executions (deterministic executions), \textit{lastChange} only requires \textit{bug 
...
analyzing large and complicated data make debugging difficult and time-consuming. 
Therefore, enhancements to debugging techniques and tools which automate 
or facilitate developers' actions on execution trace, can save developers' 
time which consequently lead to development costs reduction and programs' 
quality improvement.
...
Figure ~\ref{fig:recursive} demonstrates a case in which the same syntactically variable
...
To address this issue, we perform dependency analysis (Figure~\ref{fig:dependency-analysis}), to
...
property changes in the object. Setting off a flag\footnote[2]{DISABLE\_OBJECT\_TRACE defined in jsdIDebuggerService.idl} into the Firefox 
...
for automatic re-execution: local-reproduction and record-replay. The first one,
...
executions. Therefore, if developer specifies an execution deterministic, every lastChange

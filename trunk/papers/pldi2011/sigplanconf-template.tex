...
\subfigure[A view of Firebug JavaScript debugger. The selected script
  panel, presents all loaded JavaScript code. Developer has access to
  all loaded source file and can set breakpoints on lines. In this
  figure, the execution is paused at line 22 by a regular
  breakpoint. The panel at the right site shows the program state at
  the paused
  point. ]{\label{fig:example1}\includegraphics[width=1.0\textwidth,
    height=.19\textheight]{1-bp22.jpg}}

\subfigure[Developer can query \textit{lastChange} on a value by right
  clicking on the
  value.]{\label{fig:example2}\includegraphics[width=1.0\textwidth,
    height=.19\textheight]{2-bp22-lastChange.jpg}}

\subfigure[The result of \textit{lastChange} query for
  \texttt{myObject.myProperty}. The right panel shows the collected
  data at the

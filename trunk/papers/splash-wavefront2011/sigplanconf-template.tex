...
\begin{tabular}{ | c | l | m{2.5cm} | l|}
  ...
  \hline 
   ...
   \cline{0-0}\cline{3-4}
	 ...
	 2 & \multirow{4}{*}{\hspace{.2cm}\begin{rotate}{90}The function object is\end{rotate}} 
	 & \raggedright 
   ...
   4 &
   & \raggedright assigned to a variable or write pointer.
   & foo\textsuperscript{*} = function()\{...\} \\
   \cline{0-0}\cline{2-4} 
...
\subsection{Expression Value Consumption Summary}
\label{sec:nodewalk}
The algorithm builds a consumption summary for an expression, starting from the expression node walking up in the absstract syntax tree. In the particular case when we start from an expression wich defines a function ( a function body or {\small\texttt{new Function()}}), it calculates the consumption summary for the function. The first node is an expression and returns a value. The algorithm ends when we reach a node which is an assignment or a statmement which does not return any value. When we move from a node to its parent, the value of the node expression can be the same, part of or different from its parent expression value. For every visited node we keep this information. 
...
\WHILE {$n.parent$ is an expression}  
    ...
    \ELSE	
	   		\STATE $dataItem.isContributedTo$ = $true$
    \ENDIF
...
Once we move from a node to its parent, we keep the 

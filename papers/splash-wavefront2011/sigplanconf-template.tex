...
walk up the tree recording identifiers or expressions explains the function object consumption. The result is, \textit{object consumption summary}, a list of collected data at every visited node in the tree. %Then, to refine the summary from temporary variable, we look for assignments from and to the variables appear in the summary. Then we replace the temporary variables more expressive identifiers (e.g., property names) or strings. After the refinement step, 

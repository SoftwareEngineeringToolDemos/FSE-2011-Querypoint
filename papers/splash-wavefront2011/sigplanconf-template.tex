...
\centering
  ...
  \hline 
   ExtJS         & 30221(83\%)& 10  & 1476(4\%)  & 3  & 16 & 9  & 0 & 820(2\%) & 788(2\%) & 509(1\%) & 2617(7\%) \\ 
  \hline 
...
This case usually happens when the functions are assigned to object properties after its initialization. Similar to the first case the property name is assigned to the function body as its name. 
...
For example see the inner function in Fig.~\ref{fig:jQueryEach}. The variable
 name in the cases we examined was generic name like {\small\texttt{o}} or {\small\texttt{item}}. Nevertheless identifying the function body using the assignment target seems natural and clear in these cases.
\begin{figure*}[htp]
\begin{verbatim}
...
\subsubsection{Case 6: setProperty by Expression}
 A common case of expression in this case is a conditional expression, e.g., {\small\texttt{condition?"prop1":"prop2"}}. To name this functions, similar to previous case we use the owner object and a substring of expression inside the brackets.
...
Although this category only includes 2\% of nameless function, proper naming of functions in this class is important. Many constructors are build by this form and therefore the name of these functions appear in object summaries.
...
For example, see Fig.~\ref{fig:jQueryEach}. The calling function and the other arguments look helpful, to the extent that they have identifiers invented by the developer. As in the example we see that the calling function, \verb|jQuery.each()| can be generic so it provides less valuable information, but the arguments in the example are highly specific to the function body. 

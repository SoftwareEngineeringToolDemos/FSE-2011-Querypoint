...
\section{The Anonymous Functions Problem}
...
\section{Automated Funcation Naming}
  The problem is discussed in several locations on the web \cite{DisplayName, Zaytsev} and various solutions have been proposed by developers. A simple solution would be annotating the function by its name or adding a property (e.g., \textit{displayName}) with the function name as its value. Although these solutions may help, they cause developers extra work.
...
\subsubsection{Function Object vs. Function Body}%What Should be Named?}
The first question arises is that which one should be named? the function body or the function object. A function name must help the developer to distinguish the function from other functions with different behaviours. The behaviour of a function object, in addition to its body (the source code), is dependent to the enclosing scopes (e.g., global and closure variables). Although the actual enclosing scopes are not known until the function object creation, their structure can be almost known from the enclosing source code. Therefore, to name a function object, it is fair enough to name its \textit{function body}. 
...
%It is conceivable that two function objects which are created from the same function body have different enclosing scopes and therefore different behaviours.
...
\subsubsection{What Makes a Good Function Name?}
A function name should be useful for both the developer who are familar with the code and the developer who is not. It should explains the function behaviour, or an abstraction of the function behaviour, or why/where the function is used (e.g., to create object \textit{foo}), or at least works as an identifier.

A function body can be uniquely identified by its location in the source file. A naive proposal for the function name for a function body is a combination of the function file name and line number. Although it may work as an identifier, but it does not assist the develper to recall or understand the function behaviour. On the other extreme extend, the function body itself can be used as the function identifier. This identifier it too long to be displayed and read by the developer. 
...
\subsection{The basic idea}
   We employ the fact that developers know many function objects (particularly those which are reused) by the names they can access and call them. 
   On the other hand, it is very likely that the function object is assigned to this name in a place very close to the place the function object is created. Therefore, we analyze the code to find all names that the function object is assigned to them. For cases where the function object is not assigned to a variable or object property but is used to produce other values we keep trakc of the function effect on other identifiers.

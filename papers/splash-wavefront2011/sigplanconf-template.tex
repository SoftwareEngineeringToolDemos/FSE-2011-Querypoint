...
Our algorithm applies the same logic: we follow the function object backwards through the code to create a name in cases where an identifier is not directly given in the code. By 'backwards through the code' we mean that we parse the JavaScript and analyze the resulting syntax tree. For each function body, we walk up the tree recording identifiers or expressions explains the function object consumption. The result is, \textit{object consumption summary}, a list of collected data at every visited node in the tree. %Then, to refine the summary from temporary variable, we look for assignments from and to the variables appear in the summary. Then we replace the temporary variables more expressive identifiers (e.g., property names) or strings. After the refinement step, 

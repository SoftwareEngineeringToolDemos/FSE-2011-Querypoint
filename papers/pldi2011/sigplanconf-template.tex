...
The JavaScript prototype is implemented as an extension to Firebug\cite{Firebug} javasrcipt debugger. In this section, first we explain technical details for the \textit{lastChange} implementation skipped in the previous section. Then we address other issues, such as re-execution, data collection and presentation.
...
By parsing the source code we can find all statements that create a new object. It might be an object constructed by \texttt{\{\}} or \texttt{new ... } or spcial objects like function which can be created by \texttt{function()}.
Every recently created object might be assigned to a variable or property by \texttt{=} or \texttt{:} or be passed to a function as an argument.
For the first two case we create a list of assignees.
Having the list of possible assignees we do stepping and after every step we check those assignees values and if the value is changed, we consider the new value
 as a created object.
...
%If we assume for every change event k additionial instructions should be executed and the number of instructions executed in one execution is n and the number of change %events is m then the total amount of instructions is n+mk. The overhead is mk/n. for m=10^3, k=10^2 and n=10^10 the overhead is very low about 10^-5.
